...
- **重要：レポート形式の固定化**: 以下の構造、テーブルレイアウト、およびトーンを厳守すること。
    ...
    3. **メニュー別ログ**: **モダンなカード型レイアウト（Flexbox）を使用すること。**
       - 構造（上から順に）：
         1. **料理名**: `font-weight: 900` の太字テキスト。
         2. **写真**: **重要：HTMLの `<img>` タグを使用し、`src` には `file:///` から始まる絶対パスを指定すること。**
            - 例：`<img src="file:///Users/.../image.jpg" style="width: 100%; border-radius: 12px;">`
         3. **数値チップ**: カロリー（黒背景）とPFC（薄灰色背景）を四角いチップ状に並べる。
         4. **コメント**: 料理の内容に対する客観的な解析（「AIの解析」等の表現は使わない）。
         5. **辛酷な現実**: 薄い赤色の背景（`#fcf6f6`）に、左側の濃い赤色（`#d94841`）の太いサイドラインを付けたボックス。
       - **重要**: 辛口コメントは語彙力を駆使し、**「突き刺さるような表現」**で長めに書くこと。
