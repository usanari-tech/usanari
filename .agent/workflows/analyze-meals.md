...
- **重要：レポート形式の固定化**: 以下の構造、テーブルレイアウト、およびトーンを厳守すること。
    1. **全体ラップ**: レポート全体を `<div style="font-size: 0.85em;"> ... </div>` で囲み、情報を凝縮すること。
    2. **タイトル**: `# 🍴 栄養解析レポート (YYYY-MM-DD)`
    3. **サマリーテーブル**: エネルギー、P、F、Cの4項目。実績値と評価（絵文字付き）を含む。
    4. **メニュー別ログ**: **Markdownの標準テーブルを使用し、画像のみをヘッダー行に配置する「極シンプル構造」とすること。**
       - 構造（最高安定性 ＆ スマホ最適化 ＆ 2列表示）：
         ```markdown
         | ![[image1.jpg\|300]] | ![[image2.jpg\|300]] |
         | :--- | :--- |
         | <div style="font-size: 1.0em; line-height: 1.4;">**1. 料理名**<br>**550kcal**<br>**P15g F20g C75g**<br>**コメント :**<br>内容...<br><br>**辛辣なコメント：**<br>内容...</div> | <div style="font-size: 1.0em; line-height: 1.4;">**2. 料理名**<br>**120kcal**<br>**P8g F6g C10g**<br>**コメント :**<br>内容...<br><br>**辛辣なコメント：**<br>内容...</div> |
         ```
       - **重要事項**: 
         - **安定構造**: 画像を1行目、テキスト要素を2行目の単一 `div` に集約。
         - **画像表示**: `![[画像名\|300]]` のWiki記法を使用。
         - **フォントサイズ**: 外側を 0.85em で囲んでいるため、内側の `div` は `1.0em` で指定することで、相対的に全体を縮小コントロールする。
         - **数値情報**: PFCは `P15g F20g C30g` 形式。
         - **ラベル**: 「コメント : 」「辛辣なコメント：」を用い、直後で改行、項目間に空行。
         - 辛口コメントは語彙力を駆使し、**「突き刺さるような表現」**で長めに書くこと。
    5. **最終宣告**: `> [!CAUTION]` アラートボックスを用いた総括。
